...
if uploaded_file is not None:
    ...
    # Пора звонить + Пора звонить (холодняк) → Пора звонить (все)
    df_clean["Стадия"] = df_clean["Стадия"].replace(
        {
            "Пора звонить": "Пора звонить (все)",
            "Пора звонить (холодняк)": "Пора звонить (все)",
        }
    )
    ...
    for row in range(2, ws.max_row + 1):
        ...
        # Строки с заливкой
        elif cell_value in ["Менеджер назначен", "Пора звонить (все)"]:
            for col in range(1, ws.max_column + 1):
